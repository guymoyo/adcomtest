var module = angular.module('adcost', []);
...
angular.element(document).ready(function ($http) {
    ...
    keycloakAuth.init({ onLoad: 'login-required' }).success(function () {
        ...
        auth.logoutUrl = keycloakAuth.authServerUrl + "/realms/adcom/tokens/logout?redirect_uri=/adcost.client";
        module.factory('Auth', function() {
            return auth;
        });
        angular.bootstrap(document, ["adcost"]);
    }).error(function () {
...
module.controller('GlobalCtrl', function($scope, $http, Auth) {	
    ...
    $scope.ping = function() {
    	console.log("ping");
        $http.get("/adcost.server/rest/ping").success(function(data) {
            $scope.msg = data;
